<template>
  <a-card>
    <a-spin :spinning="loading">
      <a-row>
        <a-col span="6">
          <a-tree :auto-expand-parent="true" select-keys="selectKeys" @rightClick="rightClick" @select="treeSelect" :tree-data="treeData"></a-tree>
        </a-col>
        <a-col span="18">
          <a-row>
            <a-col span="4">
      ...
      this.form.validateFields((error, fields) => {
        if (!error) {
          ...
          update({ ...this.data, ...fields }).then(() => {
            ...
            return this.loadTreeData().finally(() => {
              this.loading = false
            })
          })
        }
