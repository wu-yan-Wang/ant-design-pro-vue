<template>
  <a-card>
    <a-row>
      <a-col span="6">
        <a-tree select-keys="sel" @rightClick="rightClick" @select="treeSelect" :tree-data="treeData"></a-tree>
      </a-col>
      <a-col span="18">
        <a-row>
          <a-col span="4">
    ...
    <add-permission :select-node="selectKeys" ref="addModal"></add-permission>
  </a-card>
  ...
  methods: {
    treeSelect (e, node) {
      if (e.length > 0) {
        ...
        this.selectNode = [{ id: data.key, title: data.props.title }]
