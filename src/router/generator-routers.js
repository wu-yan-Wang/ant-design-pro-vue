...
export const generator = (routerMap, parent) => {
  return routerMap.map(item => {
    const { permissionName, show, hideChildren, hiddenHeaderContent, target, permissionIcon } = item || {}
    const currentRouter = {
      // 如果路由设置了 path，则作为默认 path，否则 路由地址 动态拼接生成如 /dashboard/workplace
      path: item.permissionUrl || `${parent && parent.permissionUrl || ''}/${item.permissionCode}`,
      // 路由名称，建议唯一
